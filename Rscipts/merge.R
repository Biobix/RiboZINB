...
merge_files <- function(path,pattern){
	...
	for (file in file_list) {
	 	# if the merged dataset doesn't exist, create it
	  	if (!exists("dataset")){
			dataset <- read.table(file=paste(path,file, sep="/"), header=TRUE, sep="\t")

	  	} else {	# if the merged dataset does exist, append to it
			temp_dataset <-read.table(file=paste(path,file, sep="/"), header=TRUE, sep="\t")
...
merge_files_2 <- function(path,pattern){
	...
	for (file in file_list) {
	 	# if the merged dataset doesn't exist, create it
	  	if (!exists("dataset")){
			dataset <- read.table(file=paste(path,file, sep="/"), header=F, sep="\t")

	  	} else {	# if the merged dataset does exist, append to it
			temp_dataset <-read.table(file=paste(path,file, sep="/"), header=F, sep="\t")
			dataset<-rbind(dataset, temp_dataset)
			rm(temp_dataset)
	 	}
	}
